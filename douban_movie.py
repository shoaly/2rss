...
class RSS:
    ...
    def filter_links(self):
        ...
        source_item = jQuery("#nowplaying .lists .list-item")
        # print source_item
        items = {}
        for key,row in enumerate(source_item):
            title = "标题: %s, 分数: %s, 演员:%s" % (jQuery(row).attr("data-title"),jQuery(row).attr("data-score") ,jQuery(row).attr("data-actors") )
            link = "http://movie.douban.com/subject/%s/?from=playing_poster" % jQuery(row).attr("id")
...
start = time.time()
rss = RSS(encode_code="utf-8",url="http://movie.douban.com/nowplaying/beijing/",rss_title=u"豆瓣正在上映电影",output_file="douban_movie.xml",tpl="tpl.py")
